...
import {AppModDummyTreeJobListDeleteInput} from './mod-dummy-tree-job-list-delete-input';

/** Мод "DummyTree". Задания. Список. Получить. Сервис. */
@Injectable({
  providedIn: 'root'
})
export class AppModDummyTreeJobListDeleteService {
   ...
   */
  execute$(
    input: AppModDummyTreeJobListDeleteInput,
    handler: AppCoreExecutionHandler
  ): Observable<AppCoreExecutionResult> {
    const url = this.appNavigation.createAbsoluteUrlOfApi('dummy-tree/list/delete');
