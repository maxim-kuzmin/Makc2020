...
export class SplitGutterDirective {
  ...
  private static getCursor(state: 'disabled' | 'vertical' | 'horizontal'): string {
    switch (state) {
      case 'horizontal':
        return 'col-resize';
  ...
  private refreshStyle(): void {
    ...
    this.renderer.setStyle(this.elRef.nativeElement, 'cursor', SplitGutterDirective.getCursor(state));
  }
