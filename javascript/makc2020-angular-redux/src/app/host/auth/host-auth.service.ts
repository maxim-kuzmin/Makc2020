...
import {AppHostAuthCommonJobLoginInput} from './common/jobs/login/host-auth-common-job-login-input';
import {AppHostAuthCommonJobLoginJwtOutput} from './common/jobs/login/jwt/host-auth-common-job-login-jwt-output';
...
import {AppHostAuthStore} from './host-auth-store';

/** Хост. Аутентификация. Сервис. */
export abstract class AppHostAuthService extends AppCoreAuthService {
  ...
  protected constructor(
    ...
    protected appAuthStore: AppHostAuthStore,
   ...
   * @param {AppCoreLoggingService} logger Регистратор.
   * @param {AppHostAuthCommonJobLoginJwtOutput} output Данные.
   */
  protected loadLoginOutput(
    logger: AppCoreLoggingService,
    output: AppHostAuthCommonJobLoginJwtOutput
  ) {
    if (output) {
